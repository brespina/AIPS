...
def objective(trial):
    ...
    for i, (train_index, test_index) in enumerate(tscv.split(X)):
        ...
        model = RandomForestRegressor(**params)  # Define your XGBoost model
...
# Calculate NRMSE
nrmse = mean_squared_error(y_test, rf_predictions, squared=False) / (max(y_test) - min(y_test))
print('Normalized Root Mean Squared Error:', nrmse)

# Calculate RMSLE
rmsle = np.sqrt(mean_squared_log_error(y_test, rf_predictions))
print('Root Mean Squared Log Error:', rmsle)

# Calculate R^2
r_2 = r2_score(y_test, rf_predictions)
print('R^2 score:', r_2)
